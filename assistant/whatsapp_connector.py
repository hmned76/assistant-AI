...
def _envoyer_maytapi(destinataire: str, message: str) -> dict:
    """Envoie via Maytapi WhatsApp API."""
    api_key = getattr(config, "WA_MAYTAPI_API_KEY", "")
    instance_id = getattr(config, "WA_MAYTAPI_INSTANCE_ID", "")
    if not api_key or not instance_id:
        return {"statut": "simu", "a": destinataire, "contenu": message}
    url = f"https://api.maytapi.com/whatsapp/{instance_id}/sendMessage"
    resp = requests.post(
        url,
        json={"phone": destinataire, "messages": [{"message": message}]},
    ...
    if resp.status_code == 200:
        data = resp.json()
        if data.get("success"):
            return {"statut": "envoye", "a": destinataire, "id": data.get("id")}
    return {"statut": "erreur", "details": resp.text}
...
class WhatsAppMonitor:
    ...
    def _poller_maytapi(self, since_timestamp: int) -> list:
        ...
        if not api_key or not instance_id:
            return messages
        try:
            url = f"https://api.maytapi.com/whatsapp/{instance_id}/getMessages"
            resp = requests.get(
            ...
            print("WA Maytapi polling erreur:", e)
        return messages
